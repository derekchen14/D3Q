...
-- user_goals_ftt.revised.v1.p: all goals
-- user_goals_ftt.part.movie.v1.p: moviename in goal.inform_slots
-- user_goals_ftt.part.nomovie.v1.p: no moviename in goal.inform_slots
...
class RuleSimulator(UserSimulator):
    ...
    def response_request(self, system_action):
        ...
        if len(system_action['request_slots'].keys()) > 0:
            slot = next(iter(system_action['request_slots'])) # only one slot
